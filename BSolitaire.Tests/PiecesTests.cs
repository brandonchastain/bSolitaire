...
public class PiecesTests
{
    ...
    public void AFinishThatStopsMakingProgressGivesUp()
    {
        ...
        var board = Empty();
        board.Mutable(Stock).Add(Down(Suit.Hearts, Rank.Five));  // nothing it can ever go on
        board.Mutable(Tableau(0)).Add(Up(Suit.Spades, Rank.King));
        board.Mutable(Tableau(1)).Add(Up(Suit.Hearts, Rank.Queen)); // a move exists, so not stuck
    ...
    public void ProvingADealDeadIsReportedToTheBoard()
    {
        ...
        var board = Empty();
        board.Mutable(Tableau(0)).Add(Up(Suit.Spades, Rank.King));
        board.Mutable(Tableau(1)).Add(Up(Suit.Hearts, Rank.Queen));
        board.Mutable(Stock).Add(Down(Suit.Diamonds, Rank.Ace)); // an ace can always go home
        board.DealFromStock();
        Assert.Equal(GameState.Playing, board.State);
    ...
    public void ADealDealtAgainIsCountedAgain()
    {
        ...
        foreach (var kind in new[] { PileKind.Foundation, PileKind.Tableau })
        {
            for (int i = 0; i < board.PileCountOf(kind); i++)
            {
                ...
                board.Mutable(loc).Clear();
                board.Mutable(loc).AddRange(second.Pile(loc));
            }
        }

        board.Mutable(Stock).Clear();
        board.Mutable(Waste).Clear();
