...
public class PiecesTests
{
    ...
    public void AFinishThatStopsMakingProgressGivesUp()
    {
        ...
        var board = Empty();
        board.Place(Stock, Down(Suit.Hearts, Rank.Five));  // nothing it can ever go on
        board.Place(Tableau(0), Up(Suit.Spades, Rank.King));
        board.Place(Tableau(1), Up(Suit.Hearts, Rank.Queen)); // a move exists, so not stuck
    ...
    public void ProvingADealDeadIsReportedToTheBoard()
    {
        ...
        var board = Empty();
        board.Place(Tableau(0), Up(Suit.Spades, Rank.King));
        board.Place(Tableau(1), Up(Suit.Hearts, Rank.Queen));
        board.Place(Stock, Down(Suit.Diamonds, Rank.Ace)); // an ace can always go home
        board.DealFromStock();
        Assert.Equal(GameState.Playing, board.State);
    ...
    public void ADealDealtAgainIsCountedAgain()
    {
        ...
        foreach (var kind in new[] { PileKind.Foundation, PileKind.Tableau })
        {
            for (int i = 0; i < board.PileCountOf(kind); i++)
            {
                ...
                board.Strip(loc);
                board.Place(loc, second.Pile(loc));
            }
        }
